...
LogisticsStatistics.getAll = (keyValue, result) => {
    let mySql = keyValue == undefined ? `SELECT * FROM my_tracking` : `SELECT * FROM my_tracking a WHERE 1=1 and (a.trackingNumber =${keyValue} or a.orderNumber =${keyValue} or a.sku =${keyValue} or  a.date_format(trackTime, '%Y-%m-%d ')=${keyValue} or a.date_format(startTime, '%Y-%m-%d ')=${keyValue}`
